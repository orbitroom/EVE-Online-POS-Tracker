<?php
/** $Id: eveRender.class.php 87 2010-08-12 02:54:12Z frozenice2525 $
 *
 * * eveRender *
 *
...
class eveRender extends Smarty {
     ...
     */
    function eveRender($config = array(), $module = '', $caching = null)
    {
        ...
        if (is_dir('mods/'.$this->module.'/plugins')) {
            array_push($this->plugins_dir, 'mods/'.$this->module.'/plugins');
        }
        if (is_dir('themes/'.$this->theme.'/plugins')) {
            array_push($this->plugins_dir, 'themes/'.$this->theme.'/plugins');
        }
        //Theme config
        if (is_file('themes/'.$this->theme.'/themeconfig.php')) {
            include_once 'themes/'.$this->theme.'/themeconfig.php';
            $this->themeconfig = $themecfg;
        }
        //---- Cache handling -------------------------------------------------
        // use HTML cache system?
        $this->caching = (($caching <> null) ? $caching : false); //pnModGetVar('pnrender', 'cache');
        $this->cache_lifetime = 60; //pnModGetVar('pnrender', 'lifetime');

        // HTML cache directory
        $this->cache_dir = $config['filepath'] . '/templates_cached';
        ...
        $this->compile_dir = $config['filepath'].'/templates_c';

        // compile id
        $this->compile_id = $this->theme;//$config['charid']; //'';//$this -> module . '|' . $theme . '|' . pnUserGetLang();
        ...
        //$this->register_outputfilter('shorturls');
    ...
    function get_template_path($template)
    {
        ...
        $themepath = 'themes/'.$this->theme.'/mods/'.$this->module;
        $modpath   = 'mods/'.$this->module.'/templates';
        $syspath   = 'themes/'.$this->theme;

        $search_path = array($themepath, $modpath, $syspath);

        foreach ($search_path as $path) {
            if (file_exists("$path/$template") && is_readable("$path/$template")) {
                return $path;
            }
        }
    ...
    {//echo $this->get_template_path($template);exit;
